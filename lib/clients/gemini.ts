...
export async function understand(asset: MediaAsset, intent: string) {
  const result = await request(process.env.GEMINI_MODEL ?? "gemini-2.5-flash", [{ parts: [await sourcePart(asset), { text: `Describe this fashion reference for wearable object design. Return concise JSON with visualSummary, materials, colors, silhouette, and suggestedBodyRegions. Creative intent: ${intent}` }] }]);
...
export async function generatePrimitives(asset: MediaAsset, intent: string, primitives: WearablePrimitive[]) {
  const result = await request(process.env.GEMINI_IMAGE_MODEL ?? "gemini-2.5-flash-image", [{ parts: [await sourcePart(asset), { text: `Generate a clean studio concept image of a wearable fashion primitive based on this reference. Focus on ${primitives[0]?.name ?? "a wearable module"}. Intent: ${intent}.` }] }], ["TEXT", "IMAGE"]);
