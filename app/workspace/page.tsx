...
export default function Workspace() {
  const demoAsset: MediaAsset = { id: demoCadAssets[0].id, kind: "cad", name: demoCadAssets[0].name, url: demoCadAssets[0].stepUrl, mimeType: "model/step", sizeBytes: 28106, createdAt: "2026-01-01T00:00:00.000Z" };
  const [asset, setAsset] = useState<MediaAsset | null>(demoAsset);
  const [intent, setIntent] = useState("");
  const [cadUrl, setCadUrl] = useState<string | null>(null);
  const [meshUrl, setMeshUrl] = useState<string | null>(demoAsset.url);
  ...
  function loadDemo() { const demo = demoCadAssets[0]; setAsset({ id: demo.id, kind: "cad", name: demo.name, url: demo.stepUrl, mimeType: "model/step", sizeBytes: 28106, createdAt: new Date().toISOString() }); setMeshUrl(demo.stepUrl); setStatus("DEMO READY"); }
