...
export default function Workspace() {
  ...
  return <main className="shell">
    ...
    <section className="grid">
      ...
      <article className="panel forma-panel"><div className="panel-head"><span>05 / MATERIALIZE</span><b>GEMINI CAD OUTPUT</b><span className="count">{formaStatus}</span></div><div className="output-art">{formaResult ? <><span>✓</span><small>{meshResult ? "RENDERED MESH READY" : "CAD SOURCE READY"}<br />{selected.name}</small></> : <><span>◈</span><small>OPENSCAD SOURCE<br />AWAITING GENERATION</small></>}</div><button className="outline" onClick={sendToCad} disabled={!asset || !intent || formaStatus === "GENERATING CAD..."}>GENERATE AND RENDER  →</button>{formaResult && <a className="download-link" href={formaResult} download>DOWNLOAD .SCAD  →</a>}{meshResult && <a className="download-link" href={meshResult} download>DOWNLOAD .STL MESH  →</a>}</article>
    </section>
