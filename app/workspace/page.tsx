...
export default function Workspace() {
  ...
  async function upload(file: File) { setUploading(true); const form = new FormData(); form.append("file", file); const response = await fetch("/api/media", { method: "POST", body: form }); if (response.ok) { const uploaded = (await response.json()).asset as MediaAsset; setAsset(uploaded); setMeshResult(uploaded.meshUrl ?? null); } setUploading(false); }
  ...
  async function sendToCad() { if (!asset || !intent) return; setFormaStatus("GENERATING CAD..."); const response = await fetch("/api/gemini/cad", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ mediaAssetId: asset.id, intent }) }); if (response.ok) { const result = await response.json(); setFormaResult(result.downloadUrl); setMeshResult(result.meshUrl ?? null); setFormaStatus(result.meshUrl ? "MESH READY" : "SOURCE READY"); } else setFormaStatus("CAD FAILED"); }
  return <main className="shell">
    ...
    <section className="grid">
      <article className="panel input-panel"><div className="panel-head"><span>01 / INPUT</span><b>REFERENCE MEDIA</b></div><input ref={fileRef} className="file-input" type="file" accept="image/png,image/jpeg,image/webp,video/mp4,.step,.stp,.iges,.igs" onChange={e => e.target.files?.[0] && upload(e.target.files[0])} /><div role="button" tabIndex={0} className="dropzone" onClick={() => fileRef.current?.click()}><div className="upload-icon">↥</div><strong>{uploading ? "Uploading..." : "Drop a look here"}</strong><span>or browse your files</span><small>PNG, JPG, WebP, MP4, STEP up to 50MB</small></div><div className="sample-list"><small>TRY A SAMPLE</small>{sampleAssets.map(sample => <button key={sample.id} onClick={() => useSample(sample)}>{sample.name} ↗</button>)}</div>{asset && <div className="asset-row"><img className="thumb-image" src={asset.kind === "video" ? asset.thumbnailUrl : asset.url} alt={asset.kind === "video" ? "Video thumbnail" : asset.name} /><span><b>{asset.name}</b><small>{asset.kind.toUpperCase()} / {asset.sizeBytes ? `${(asset.sizeBytes / 1024 / 1024).toFixed(1)} MB` : "SAMPLE"}</small></span><span className="check">✓</span></div>}</article>
      ...
      <article className="panel forma-panel"><div className="panel-head"><span>05 / MATERIALIZE</span><b>GEMINI CAD OUTPUT</b><span className="count">{formaStatus}</span></div>{meshResult ? <CadViewport url={meshResult} /> : <div className="output-art"><span>◈</span><small>OPENSCAD SOURCE<br />AWAITING RENDER</small></div>}<button className="outline" onClick={sendToCad} disabled={!asset || !intent || formaStatus === "GENERATING CAD..."}>GENERATE AND RENDER  →</button>{formaResult && <a className="download-link" href={formaResult} download>DOWNLOAD .SCAD  →</a>}{meshResult && <a className="download-link" href={meshResult} download>DOWNLOAD .STL MESH  →</a>}</article>
    </section>
