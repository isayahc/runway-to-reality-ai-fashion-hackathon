...
export default function Workspace() {
  ...
  return <main className="shell">
    ...
    <section className="grid">
      ...
      <article className="panel primitives-panel"><div className="panel-head"><span>04 / TRANSLATION</span><b>WEARABLE PRIMITIVES</b><span className="count">{primitives.length} FOUND</span></div>{primitives.map(item => <div className="primitive" key={item.id}><img className="primitive-image" src={item.imageUrl} alt="" /><span className="primitive-id">{item.id.slice(-2)}</span><span><b>{item.name}</b><small>{item.targetBodyRegion.toUpperCase()} / {Math.round(item.confidence * 100)}% CONFIDENCE</small></span><span className="arrow">↗</span></div>)}</article>
